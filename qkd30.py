# qkd30.py  — 段階30：ファイバ + 衛星 + 天候相関(ガウス・コピュラ) + CHSHゲート + バッファ + 消費
...
def chsh_pass_once(qber: float, n_pairs: int, rng: np.random.Generator) -> bool:
    """
    QBERとサンプル数からCHSH検定が通るかを簡便に判定（教育用近似）。
    期待値 S ≈ 2√2 * (1 - 2Q) として、閾値 2 を超える確率を正規近似で評価。
    分散は 1/√N に比例とした粗いモデル。
    """
    if n_pairs <= 0:
        return False
...
SAT_QBER           = 0.03                  # 衛星リンクのQBER（CHSHモデル用）
...
# CHSHゲート：テストに使う比率（残りが鍵用）
CHSH_TEST_FRACTION = 0.2                   # 20%テスト／80%鍵、など
...
def main():
    ...
    ax[0].plot(days, prod_fiber_day/1e9, label="Fiber produced [Gbit/day]")
    ax[0].plot(days, prod_sat_day/1e9,   label="Satellite (CHSH-gated) [Gbit/day]")
    ax[0].plot(days, cons_day/1e9,       label="Consumed [Gbit/day]")
    ax[0].set_ylabel("Gbit/day"); ax[0].grid(True); ax[0].legend(loc="upper right")

    ax[1].step(days, outage_minutes_day, where='mid', label="Outage minutes [min/day]")
    ax[1].set_xlabel("Day"); ax[1].set_ylabel("Minutes"); ax[1].grid(True); ax[1].legend(loc="upper right")
    fig.suptitle("Daily minima & outage time (CHSH-gated)")

    plt.tight_layout(); plt.show()

    # ===== KPI =====
    print(f"\n=== KPI ({DAYS} days, CHSH-gated) ===")
    ...
    print(f"Median of daily min buffer: {np.median(min_buffer_day)/1e6:.1f} Mbit")
    print("Notes: Satellite production is admitted only if BOTH weather and CHSH pass.")
    print("Tips: Increase N_SAT / lower CHSH_TEST_FRACTION to change pass/key balance; adjust SAT_QBER.")
