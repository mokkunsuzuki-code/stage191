...
def wilson_CI(k:int,n:int,alpha:float)->tuple[float,float]:
    ...
    p=k/n; z=normal_ppf(1-alpha/2); den=1+z*z/n
    ctr=(p+z*z/(2*n))/den
...
def sha256_amp(bits: np.ndarray, m:int)->np.ndarray:
    if m<=0: return np.zeros(0,dtype=np.uint8)
    raw=bits_to_bytes(bits); buf=bytearray(); ctr=0
    while len(buf)*8<m:
        buf.extend(hashlib.sha256(raw+ctr.to_bytes(4,'big')).digest()); ctr+=1
    bitstr=''.join(f'{b:08b}' for b in buf)[:m]
    return np.fromiter((1 if c=='1' else 0 for c in bitstr), dtype=np.uint8)

# ===== E91（教育） =====
A=[0.0, math.pi/4]; B=[math.pi/8, -math.pi/8]
...
def run_e91(N_pairs=400_000, key_fraction=0.85, p_flip=0.004, alpha_CI=0.02, seed=2025):
    ...
    a_key=np.array(a_key,dtype=np.uint8); b_key=np.array(b_key,dtype=np.uint8)
    n_key=len(a_key); qber=float(np.mean(a_key ^ b_key)) if n_key>0 else 0.0

    E_pt={}; E_lb={}
    for ai,bi in [(0,0),(0,1),(1,0),(1,1)]:
        eq,tot=ch[(ai,bi)]
        if tot==0: E_pt[(ai,bi)]=0.0; E_lb[(ai,bi)]=-1.0
        else:
            p_eq=eq/tot
            E_pt[(ai,bi)]=2*p_eq-1
    ...
    return {"a_key":a_key, "b_key":b_key, "n_key":n_key, "qber":qber, "S_pt":S_pt, "S_LB":S_LB}
...
def cascade(a_key:np.ndarray,b_key:np.ndarray, passes=(256,128,64,32,16,8,4,2,1), extra=2, seed=2025):
    rng=np.random.default_rng(seed)
    ...
    def one_pass(bs:int):
        ...
        for s in range(0,n,bs):
            ...
            if parity(a[s:e])!=parity(b[s:e]):
                leak+=1; leak+=bs_fix(a,b,s,e)

    for bs in passes:
        perm=rng.permutation(n); a=a[perm]; b=b[perm]
        one_pass(bs)
        inv=np.empty_like(perm); inv[perm]=np.arange(n)
        a=a[inv]; b=b[inv]
    for _ in range(extra): one_pass(1)
...
def consume_tag(bits:np.ndarray, tag_bits=128)->tuple[np.ndarray,int,bool]:
    if len(bits)<tag_bits: return bits.copy(),0,False
...
def pipeline(N_pairs=400_000, key_fraction=0.85, p_flip=0.004, alpha_CI=0.02,
    ...
    leak_tag=max(leak_tag_a, leak_tag_b); auth_ok=(mism==0 and ok_a and ok_b)

    # PA
    n_after=len(a_tag); leak_total=leak_ec+leak_tag+safety_bits
    m=max(0, n_after - leak_total)
    a_final=sha256_amp(a_tag, m); b_final=sha256_amp(b_tag, m)
